...
template<> iphdr ntoh<iphdr>(const u_int8_t* data)
{
  iphdr ip = *reinterpret_cast<const iphdr*>(data);
...
template<> udphdr ntoh<udphdr>(const u_int8_t* data)
{
  udphdr udp = *reinterpret_cast<const udphdr*>(data);
...
template<class T> unsigned ntoh_adv(const u_int8_t*& data, T& t)
{
  t = ntoh<T>(data);
  data += sizeof(T);
  return sizeof(T);
...
isCommand(const ether_header& mac, const llc_header& llc)
{
  ...
  for(vector<pair<ether_header,bool> >::iterator i = m_conversations.begin();
      i!=m_conversations.end(); i++)
    {
      if(memcmp(&e1, &i->first, sizeof(e1))==0)return true;
      if(memcmp(&e2, &i->first, sizeof(e2))==0)return false;
    }
  assert(0);
}
				    ...
				    const u_char* data)
{
  ...
  ether_header mac;
  ntoh_adv(data, mac);  
  if(mac.ether_type != ETHERTYPE_IP)return;
   
  iphdr ip;
  datalen = ntoh_adv(data, ip);
  datalen = ip.tot_len - datalen;
  if(ip.protocol != IPPROTO_UDP)return;

  udphdr udp;
  datalen -= ntoh_adv(data, udp);

  llc_header llc;
  datalen -= ntoh_adv<llc_header>(data, llc);
  std::swap(llc.llc1_control,llc.llc3_control);
      ...
      while(datalen)
	{
	  ...
	  switch(command)
	    ...
	    case 0x05:
	      ...
	      // run on
	    case 0x04:
	      cout << "Book module: S=" << int(modifier);
	      break;
	    ...
	    case 0x07:
	      ...
	      // run on
	    case 0x06:
	      cout << "Book LAM: S=" << int(modifier);
	      break;
	    case 0x08:
	      cout << "Attach to LAM: S=" << int(modifier);
	      break;
	    ...
	    case 16:
	      cout << "Set LAM access mode = " << int(modifier);
	      break;

	    case 17:
	    case 18:
	      cout << ((command==14)?"Clear":"Test") << " LAM S=" 
		   << int(modifier);
	      break;

	    case 19:
	      cout << "Inform host of LAM S=" << int(modifier);
	      break;
